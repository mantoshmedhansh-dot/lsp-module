...
export default function QCExecutionDetailPage() {
  ...
  const handleStartExecution = async () => {
    try {
      const response = await fetch(`/api/v1/qc/executions/${executionId}/start`, {
        method: "POST",
      });
  ...
  const handleSaveResults = async () => {
    try {
      setSaving(true);
      const response = await fetch(`/api/v1/qc/executions/${executionId}/results`, {
        method: "PUT",
  ...
  const handlePutOnHold = async () => {
    try {
      const response = await fetch(`/api/v1/qc/executions/${executionId}/hold`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
