"""
JioMart Partner API Adapter
Implementation of MarketplaceAdapter for JioMart Seller/Partner API
"""
...
JIOMART_BASE_URL = "https://seller.jiomart.com/api/v1"


@register_adapter("JIOMART")
class JioMartAdapter(MarketplaceAdapter):
    """
    JioMart Partner API Adapter.

    Implements JioMart Seller API for:
    - Orders API (order fetch, dispatch, cancellation)
    - Inventory API (bulk stock updates)
    - Returns API (return handling)
    - Settlements API (financial data)

    Authentication: API Key in X-Api-Key header (no OAuth).
    """

    def __init__(self, config: MarketplaceConfig):
        super().__init__(config)
        self.base_url = config.api_endpoint or JIOMART_BASE_URL
    ...
    def _auth_headers(self) -> Dict[str, str]:
        """Build authentication headers with API Key."""
        return {
    ...
    async def _make_request(
        ...
        """Make authenticated request to JioMart API."""
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                raise Exception(
                    f"JioMart API error {response.status_code}: "
            ...
            raise Exception("JioMart API request timed out")
        except httpx.RequestError as e:
            raise Exception(f"JioMart API request failed: {e}")
    ...
    async def authenticate(self) -> AuthResult:
        """
        Validate API Key credentials with JioMart.

        JioMart uses API Key authentication via X-Api-Key header (no OAuth).
        ...
        try:
            if not self.credentials.api_key:
                return AuthResult(
                    success=False,
                    error_message="API Key is required for JioMart",
                )
            ...
            if response.status_code == 401 or response.status_code == 403:
                return AuthResult(
                    success=False,
                    error_message="Invalid JioMart API Key",
                )
            ...
            logger.error(f"JioMart authentication failed: {e}")
    ...
    async def refresh_token(self) -> AuthResult:
        ...
        JioMart uses a static API Key; no token refresh is needed.
    ...
    async def fetch_orders(
        ...
        """Fetch orders from JioMart with date range filters."""
        try:
            params: Dict[str, Any] = {
    ...
    def _parse_order(self, data: Dict) -> Optional[MarketplaceOrder]:
        """Parse a JioMart order response into the standardized MarketplaceOrder."""
        try:
            ...
            logger.error(f"Failed to parse JioMart order: {e}")
            return None

    async def get_order(self, marketplace_order_id: str) -> Optional[MarketplaceOrder]:
        """Get a specific order by ID from JioMart."""
        try:
    ...
    async def update_order_status(self, update: OrderStatusUpdate) -> bool:
        """
        Dispatch/ship an order on JioMart.

        Uses POST /orders/{id}/dispatch with tracking information.
        """
        try:
            body: Dict[str, Any] = {
    ...
    async def push_inventory(
        ...
        """Push inventory updates to JioMart via PUT /inventory/bulk-update."""
        ...
        try:
            ...
            response = await self._make_request("PUT", "/inventory/bulk-update", body=payload)
            ...
            for update in updates:
                ...
                results.append(InventoryUpdateResult(
                    ...
                    success=success,
                    previous_qty=item_resp.get("previous_quantity") if item_resp else None,
                    new_qty=update.quantity if success else None,
                    acknowledged_qty=item_resp.get("acknowledged_qty") if item_resp else None,
    ...
    async def get_inventory(self, marketplace_skus: List[str]) -> Dict[str, int]:
        """Get current inventory levels from JioMart."""
    ...
    async def fetch_settlements(
        ...
        """Fetch settlement/payment reports from JioMart."""
    ...
    async def fetch_returns(
        ...
        """Fetch return requests from JioMart via GET /returns."""
    ...
    async def update_return_status(
        ...
        """Update return status on JioMart."""
        try:
            body: Dict[str, Any] = {
                "status": status,
            }
    ...
    async def health_check(self) -> bool:
        """Check if JioMart connection is healthy."""
        try:
            client = await self._get_client()
            url = f"{self.base_url}/orders"
            headers = self._auth_headers()
            ...
            logger.error(f"JioMart health check failed: {e}")
            return False
